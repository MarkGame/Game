...
local scheduler = mtSchedulerMgr()
...
function SocketTCP:connect(__host, __port, __retryConnectWhenFailure )
	...
	if not __checkConnect() then
		...
		local __connectTimeTick = function ()
            ...
            printInfo("%f.connectTimeTick %f", self.waitConnect, SOCKET_CONNECT_FAIL_TIMEOUT );
		...
		self.connectTimeTickScheduler = scheduler:addScheduler(SOCKET_RECONNECT_TIME,-1,__connectTimeTick)
        --self.connectTimeTickScheduler = TimeService:setTimer( SOCKET_TICK_TIME, 0, __connectTimeTick );
	end
end 
        ...
        if totalSize ~= 0 then
            ...
            if nil ~= error then
                ...
                if error == STATUS_TIMEOUT or error == STATUS_NOT_CONNECTED then
                    ...
                    printInfo("分拆网络包,包的总的大小=%f,发送大小=%f", totalSize, sentSize );
                --网络已经关闭
                --其他的未知错误
                --STATUS_CLOSED
                else
					mtEventDispatch():dispatchEvent(SocketTCP.EVENT_CLOSE_REASON,{name=SocketTCP.EVENT_CLOSE_REASON, data=error, point = "send",ip = self.host, port = self.port } );
                    printInfo("网络发送故障，清空发送缓冲区，错误为"..error );
                    self:__clearSocketBuffer();
                    self:__closeSocket();
                end
            else
                --发送正常
                print("send buffer success")
                self:__clearSocketBuffer();
            end
        end
    end
...
function SocketTCP:unscheduleTicks()
    if self.connectTimeTickScheduler then 
        scheduler:removeScheduler(self.connectTimeTickScheduler)
        self.connectTimeTickScheduler = nil; 
    end
	if self.tickScheduler then 
        scheduler:removeScheduler(self.tickScheduler) 
...
function SocketTCP:close( ... )
	...
	mtEventDispatch():dispatchEvent(SocketTCP.EVENT_CLOSE,{name=SocketTCP.EVENT_CLOSE})
end
...
function SocketTCP:disconnect()
    ...
    printInfo("SocketTCP:disconnect");
end
...
function SocketTCP:_connect()
	...
	--printInfo(self.host.."SocketTCP._connect:".. tostring(__succ).. tostring(__status))
	return __succ == 1 or __status == STATUS_ALREADY_CONNECTED
end

function SocketTCP:_disconnect()
    self:__clearSocketBuffer();
    printInfo("%s.SocketTCP:_disconnect", self.name);
	if nil == self.tcp then
		return;
	end
	self.tcp:shutdown()
	--mtEventDispatch():dispatchEvent({name=SocketTCP.EVENT_CLOSED})
end

function SocketTCP:__closeSocket()
    self:close()
	if self.isConnected then
        printInfo("%s.closed(%s, %d)", self.name, self.host, self.port)
		self:_onDisconnect()
	else
        printInfo("%s.failed(%s, %d)", self.name, self.host, self.port)
		self:_connectFailure()
	end
    self:__clearSocketBuffer();
...
function SocketTCP:checkNetworkState()
	...
	while true do
    	...
    	if __status == STATUS_CLOSED or __status == STATUS_NOT_CONNECTED then
		    self:__closeSocket();
			mtEventDispatch():dispatchEvent(SocketTCP.EVENT_CLOSE_REASON,{name=SocketTCP.EVENT_CLOSE_REASON, data=__status, point = "receive",ip = self.host, port = self.port } );
		   	return
	    end
		...
		mtEventDispatch():dispatchEvent(SocketTCP.EVENT_DATA,{name=SocketTCP.EVENT_DATA, data=(__partial or __body), partial=__partial, body=__body ,socketID = self._socketIndex })
	end
end
...
function SocketTCP:_onConnected()
	...
	mtEventDispatch():dispatchEvent(SocketTCP.EVENT_CONNECTED,{name=SocketTCP.EVENT_CONNECTED,socketID = self._socketIndex})
	if self.connectTimeTickScheduler then 
        scheduler:removeScheduler(self.connectTimeTickScheduler);
        self.connectTimeTickScheduler = nil;
    end
	-- start to read TCP data
	self.tickScheduler = scheduler:addScheduler(SOCKET_TICK_TIME,-1,handler(self, self.checkNetworkState))
end
...
function SocketTCP:_onDisconnect()
    ...
    self:unscheduleTicks();
	self.isConnected = false;
	mtEventDispatch():dispatchEvent(SocketTCP.EVENT_CLOSED,{name=SocketTCP.EVENT_CLOSED})
	--self:_reconnect();
end
function SocketTCP:_connectFailure(status)
	printInfo("%s._connectFailure", self.name);
    self:unscheduleTicks();
	mtEventDispatch():dispatchEvent(SocketTCP.EVENT_CONNECT_FAILURE,{name=SocketTCP.EVENT_CONNECT_FAILURE,socketID = self._socketIndex})
	self:_reconnect();
    self.isConnected = false;
end
...
function SocketTCP:_reconnect(__immediately)
	...
	if self.reconnectScheduler then scheduler:removeScheduler(self.reconnectScheduler) end
	local __doReConnect = function ()
		self:connect()
	end
	self.reconnectScheduler = scheduler:addScheduler(SOCKET_RECONNECT_TIME,1,__doReConnect)
end
