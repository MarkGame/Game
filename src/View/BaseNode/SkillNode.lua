...
function SkillNode:dispatchEvent(event,data)
	   mtEventDispatch():dispatchEvent(event,data)
end


function SkillNode:registerEvent( event,callBack,groupID,priority )
...
function SkillNode:onHide(  )
	
end

--设置 技能的释放范围
-- function SkillNode:setSkillRangeDiagram(diagram)
...
--刷新当前的移动动画
--自动寻路的时候 使用
function SkillNode:refreshMoveAnim(direction)
...
function SkillNode:initAnim(resName,animName)
    self.animNameList = {}
...
function SkillNode:playAnim( animType )
...
function SkillNode:initBaseInfo( constantSpeed )
...
function SkillNode:decideDirection(nextPos)
...
--强制停止寻路
function SkillNode:stopMoveToward( )
...
--callBack 最终的回调
--callBackStep 分步回调
function SkillNode:moveToward(target,callBack,callBackStep)
...
function SkillNode:walkableAdjacentTilesCoordForTileCoord(tileCoord)
  local tmp = {} 
  --up
...
function SkillNode:insertInOpenSteps(step)
...
function SkillNode:getStepIndex(steps,step)
...
function SkillNode:constructPathAndStartAnimationFromStep(step,callBack,callBackStep)
...
function SkillNode:popStepAndAnimate(callBack,callBackStep)
